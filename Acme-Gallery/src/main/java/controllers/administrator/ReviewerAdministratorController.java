...
@Controller
@RequestMapping("/reviewer/administrator")
public class ReviewerAdministratorController extends AbstractController {
	...
	public ModelAndView save(final ActorRegistrationForm actorRegistrationForm, final BindingResult binding) {
		...
		ModelAndView res;
		final Reviewer reviewerToRegister;

		reviewerToRegister = this.reviewerService.reconstructRegisterForm(actorRegistrationForm, binding);
		...
		if (binding.hasErrors())
			...
			try {
				...
				this.reviewerService.register(reviewerToRegister);
