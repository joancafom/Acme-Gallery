...
<display:table name="incidents" id="incident" requestURI="incident/${actorWS}listMine.do" pagesize="5" class="displaytag" style="width:100%" partialList="true" size="${resultSize}">
	...
	<display:column>
		<jstl:if test="${incident.isChecked eq false}">
			<a href="incident/director/check.do?incidentId=${incident.id}&listMine=true"><spring:message code="incident.check" /></a>
		</jstl:if>
	</display:column>
	...
	<display:column>
		<jstl:if test="${incident.isChecked eq false}">
			<a href="incident/director/delete.do?incidentId=${incident.id}&listMine=true"><spring:message code="incident.delete" /></a>
		</jstl:if>
	</display:column>
